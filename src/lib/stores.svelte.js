...
function createStore() {
	...
	async function loadAll() {
		...
		for (const room of rooms) {
			room.facility_ids = roomFacilities
				.filter((rf) => rf.room_id === room.id)
				.map((rf) => rf.facility_id);
		}
	...
	return {
		...
		async updateFacility(id, data) {
			...
			await addHistoryRecord('facility_updated', {
				facility: facilities.find((f) => f.id === id)?.name
			});
		},
		...
		async addRoom(r) {
			...
			if (facility_ids?.length > 0) {
				await supabase
					...
					.insert(facility_ids.map((fid) => ({ room_id: row.id, facility_id: fid })));
			}
			const newRoom = {
				...row,
		...
		async updateRoom(id, data) {
			...
			if (facility_ids !== undefined) {
				...
				if (facility_ids.length > 0) {
					await supabase
						.from('room_facilities')
						.insert(facility_ids.map((fid) => ({ room_id: id, facility_id: fid })));
				}
			}
		...
		get bookings() {
			return bookings.map((b) => {
				...
				// Optional: if it's explicitly completed or cancelled in the DB, respect that.
		...
		async addBooking(b) {
			...
			await addHistoryRecord('booking_created', {
				booking_id: newB.id,
				user: user?.full_name,
				room: room?.name
			});
		...
		async updateBooking(id, data) {
			...
			if (idx !== -1)
				bookings[idx] = {
					...bookings[idx],
					...data,
					amount: Number(data.amount ?? bookings[idx].amount)
				};
		...
		async payMonthly(paymentId, method) {
			...
			if (pIdx !== -1)
				payments[pIdx] = { ...payments[pIdx], status: 'paid', method, paid_at: paidAt };

			await addHistoryRecord('payment_received', {
			...
			if (booking) {
				...
				await supabase
					.from('bookings')
					.update({ end_date: newEndStr, status: 'pending' })
					.eq('id', booking.id);
