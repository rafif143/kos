...
export async function POST({ request }) {
	try {
		const { paymentId, amount, payerEmail, description, successRedirectUrl, failureRedirectUrl } =
			await request.json();
		...
		const invoice = await xendit.Invoice.createInvoice({
			data: {
				...
				successRedirectUrl:
					successRedirectUrl ||
					`${env.PUBLIC_SITE_URL || 'https://kos-management.netlify.app'}/bookings?payment=success`,
				failureRedirectUrl:
					failureRedirectUrl ||
					`${env.PUBLIC_SITE_URL || 'https://kos-management.netlify.app'}/bookings?payment=failed`,
				currency: 'IDR'
			}
		});
